...
<?php
	...
	echo "<tr><td style='background-color: #ccc;'></td><td style='text-align: right;background-color: #ccc;''>Total Price: <b>EGP".$total."</b></td></tr>";
	echo "</table>";
	echo "</div>";
...
if(isset($_POST['submitButton'])){
	if (empty($_POST["name"])) {
		...
		if (!preg_match("/^[a-zA-Z ]*$/", $name)){
			...
			$name = $_POST['name'];
			if (empty($_POST["ic"])){
				$icErr = "Please enter your National ID";
			}else{
				if(!preg_match("/^[0-9 -]*$/", $ic)){
					$icErr = "Please enter a valid National ID";
					$ic = "";
				}else{
					$ic = $_POST['ic'];
...
if(!isset($_SESSION['id'])){
	...
	echo 'ID Number:<br><input type="text" name="ic" placeholder="xxxxxx-xx-xxxx">';
